...
const migrateConfig = {
  ...
  '207': (state: RootState) => {
    try {
      if (state.settings && state.settings.sidebarIcons) {
        // Add 'workspace' to visible icons if not already present
        if (!state.settings.sidebarIcons.visible.includes('workspace')) {
          // Insert after 'assistants' if present, otherwise append
          const assistantsIndex = state.settings.sidebarIcons.visible.indexOf('assistants')
          if (assistantsIndex !== -1) {
            state.settings.sidebarIcons.visible = [
            ...
            state.settings.sidebarIcons.visible = [...state.settings.sidebarIcons.visible, 'workspace']
          }
        }
      }

      // Add 'workspace' tab if not already present
      if (state.tabs && !state.tabs.tabs.some((tab: { id: string }) => tab.id === 'workspace')) {
        const homeIndex = state.tabs.tabs.findIndex((tab: { id: string }) => tab.id === 'home')
        const insertIndex = homeIndex !== -1 ? homeIndex + 1 : state.tabs.tabs.length
